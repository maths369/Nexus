...
class _FakeFeishuAdapter:
    ...
    def subscription_mode(self) -> str:
        return str(self._config.get("subscription_mode", "webhook"))
...
def test_app_lifespan_starts_and_stops_feishu_long_connection(monkeypatch, tmp_path):
    app_module = importlib.import_module("nexus.api.app")
    starts: list[tuple] = []
    stops: list[str] = []

    class _FakeLongConnRunner:
    ...
    monkeypatch.setattr(app_module, "FeishuAdapter", _FakeFeishuAdapter)
    monkeypatch.setattr(app_module, "FeishuLongConnectionRunner", _FakeLongConnRunner)
